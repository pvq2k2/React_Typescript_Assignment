...
const SliderAdd = (props: SliderAddProps) => {
  ...
  const onSubmit: SubmitHandler<TypeInputs> = async ( data ) => {
    ...
    const openNotification = () => {
      notification.success({
        message: `Thêm ảnh thành công thành công !`,
      });
    };
    props.onAdd(data);
    openNotification();
  ...
  <div>
    ...
    <Breadcrumb.Item>Sliders Add</Breadcrumb.Item>
  </Breadcrumb>
<div>
  <header className="bg-white shadow">
    <div className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8 flex justify-between">
      ...
      <Link to="/admin/slider" className="sm:ml-3">
                ...
                {img && (
                        <div>
                          <label htmlFor="imgpreview" className="block text-sm font-medium text-gray-700">
                              ...
                              <div className="mt-1">
                                <img src={img.preview} className="w-[700px] rounded-[10px]"/>
                              </div>
                            </div>
                )}
            <div>
